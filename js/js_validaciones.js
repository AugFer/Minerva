...
function ejecutar(){
    var estadoActual = "q0";
    var numero = document.getElementById("input").value.toUpperCase();
    document.getElementById("numero").innerHTML = numero; // se agrega el numero ingresado al campo de la tabla resultados
    document.getElementById("input").value = ""; // limpia el input principal
    var letra;
    var recorrido = "q0 -> ";

    while(numero.length !== 0 && estadoActual !== "R"){
        letra = numero.charAt(0);
        estadoActual = transicion(estadoActual, letra);
        recorrido += estadoActual+" -> ";
        numero = numero.substring(1);
    }
    recorrido = recorrido.substring(0, recorrido.length - 4); // le quita el último " -> " (4 caracteres) que se agregan en cada pasada del while anterior
